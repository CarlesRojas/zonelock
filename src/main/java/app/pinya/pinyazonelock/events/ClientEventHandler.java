...
public class ClientEventHandler {
    ...
    private static final int PARTICLE_RENDER_RADIUS = 16;

    private static final int PARTICLE_SPAWN_INTERVAL = 5; // 20 ticks = 1 second
    ...
    private static void emitFace(ClientLevel level, Vec3 camera, Vec3 min, Vec3 max, Face f) {
        double x, y, z;
        switch (f) {
            case TOP -> {
                y = max.y;
                for (double dx = min.x; dx <= max.x; dx += STEP) {
                    for (double dz = min.z; dz <= max.z; dz += STEP) {
                        if (camera.distanceToSqr(dx, y, dz) <= PARTICLE_RENDER_RADIUS * PARTICLE_RENDER_RADIUS)
            ...
            case NORTH -> {
                ...
                for (double dx = min.x; dx <= max.x; dx += STEP) {
                    for (double dy = min.y; dy <= max.y; dy += STEP) {
                        if (camera.distanceToSqr(dx, dy, z) <= PARTICLE_RENDER_RADIUS * PARTICLE_RENDER_RADIUS)
            ...
            case WEST -> {
                ...
                for (double dz = min.z; dz <= max.z; dz += STEP) {
                    for (double dy = min.y; dy <= max.y; dy += STEP) {
                        if (camera.distanceToSqr(x, dy, dz) <= PARTICLE_RENDER_RADIUS * PARTICLE_RENDER_RADIUS)
